...
class calibrateHandInEye(object):
    def R_T2HomogeneousMatrix(self, R, T) -> np.array :
        '''
            将旋转矩阵和平移向量转换为齐次矩阵
    ...
    def HomogeneousMatrix2RT(self, Mat) -> (np.array, np.array) :
        '''将齐次变换矩阵拆分为旋转矩阵和平移矩阵
    ...
    def eulerAngleToRotationMatrix(self, eulerAngle, seq) -> np.array:
    ...
    def isRotaMatrix(self, Mat) -> bool:
...
util = calibrateHandInEye()
# print(data.calPose[:3,:3], np.transpose([data.calPose[4,:3]]), sep='\n')
# temp = util.R_T2HomogeneousMatrix(data.calPose[:3,:3], np.transpose([data.calPose[4,:3]]))
# print(temp)
rotMat = util.eulerAngleToRotationMatrix(data.calPose[0, 3:], "xyz")
print(rotMat)


import scipy_Calibrate
import data

print(scipy_Calibrate.euler2rotation(data.calPose[0, 3:]))